...
def test_all_news_variants_use_same_pipeline() -> None:
    profiles = [
        ...
        load_news_profile(
            CONFIG_DIR / "profiles" / "news-alternative.yaml"
        ),
        load_news_profile(
            CONFIG_DIR / "profiles" / "news-german.yaml"
        ),
    ]
...
def test_news_profiles_use_cheap_ranker_alias() -> None:
    profiles = [
        load_news_profile(path)
        for path in sorted((CONFIG_DIR / "profiles").glob("news-*.yaml"))
    ]

    assert all(
        profile.ranking.model_alias == "rank-cheap"
