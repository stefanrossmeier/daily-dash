...
from daily_dash.retrieval.weekend_markets import IgWeekendMarketRetriever
from daily_dash.storage import JsonFileWeekendMarketSnapshotStore


def _run(args: argparse.Namespace) -> None:
    try:
        profile = load_weekend_markets_profile(
            args.config_dir / "profiles" / f"{args.profile}.yaml"
        )
        source_set = load_weekend_market_source_set(
            args.config_dir / "sources" / f"{profile.source_set}.yaml"
        )
    except ConfigurationError as exc:
        raise SystemExit(f"Configuration invalid: {exc}") from exc

    document, output_path = run_weekend_markets_pipeline(
        profile,
        source_set,
        IgWeekendMarketRetriever(),
        snapshot_store=JsonFileWeekendMarketSnapshotStore(args.data_repo),
    )
    print(
        json.dumps(
            {
                ...
                "quote_count": len(document.report.quotes),
...
def _deliver(args: argparse.Namespace) -> None:
    document = JsonFileWeekendMarketSnapshotStore.read(args.artifact)
    profile = load_weekend_markets_profile(
        args.config_dir / "profiles" / f"{document.report.profile}.yaml"
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DailyDash weekend Markets runtime command")
    subparsers = parser.add_subparsers(dest="command", required=True)

    run_parser = subparsers.add_parser("run")
    run_parser.add_argument("--profile", choices=("markets-weekend",), default="markets-weekend")
