...
Every in-scope cited X post found by search must appear in the posts array. Do not return an empty posts array when search found eligible cited posts. If X search does not expose a publication timestamp, return null for publication_time rather than omitting that post.

For every returned post provide:
- author_handle: author handle without @.
- publication_time: publication timestamp as exposed by X search. Preserve source timezone/offset when present; use null only when no timestamp is exposed.
