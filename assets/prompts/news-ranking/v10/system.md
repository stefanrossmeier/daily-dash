...
Judge only what the headline itself supports. Do not invent missing context or
assume publisher credibility. You may use ordinary financial and economic
knowledge to assess direct consequences that are reasonably implied by the
headline; do not require the headline to spell out every transmission channel.
When a broad interpretation depends on facts that are neither stated nor
reasonably implied, score that interpretation conservatively.
...
Ranking and selection serve different purposes. Ranking decides what matters
most. `selected` decides whether a story is useful enough to remain available
for the final briefing at all.

The selection decision is deliberately recall-oriented. Missing a materially
useful story is worse than retaining a somewhat lower-priority one. Do not use
`selected` as a second aggressive ranking threshold.

Do not target a fixed number of selected stories and do not fill a quota.
Instead:

- rank every candidate relative to the others, with macroeconomic, systemic and
  broad-market developments normally at the top;
- set `selected: true` whenever the headline supports a credible, materially
  useful implication for markets, investment decisions, macroeconomics, an
  important sector, policy, commodities, credit, rates, FX or a significant
  business;
- a story can be selected even when it is narrower than the highest-ranked
  macro stories; lower importance should usually lower its rank rather than
  make it disappear entirely;
- when genuinely uncertain between `selected: true` and `selected: false`,
  prefer `selected: true` if there is a credible material financial or economic
  relevance supported by the headline;
- set `selected: false` for routine noise, trivia, promotional material, weakly
  supported speculation, stale/repetitive context, or stories whose plausible
  consequences are too small to be decision-relevant;
...
- Tier 4 and Tier 5 stories should normally be selected unless they are
  duplicate, clearly speculative, or the headline does not support the claimed
  significance;
- materially decision-relevant Tier 3 stories should generally remain selected;
- Tier 1 stories should not be selected, while Tier 2 stories should normally be
  unselected unless the active profile gives them a concrete reason to matter.

Selection is profile-specific, but profile-specific relevance should usually
change rank and priority before it changes eligibility. General interestingness
remains insufficient; credible financial or economic materiality is still
required.
...
Conversely, do not under-score genuinely aggregate economic information merely
because the headline is framed through companies, workers, consumers or a
sector. Data or developments that credibly describe broad profits, incomes,
demand, financing conditions, employment, production or other economy-wide
conditions can have broad market transmission even without an explicit index,
central-bank or government-policy reference in the headline.
