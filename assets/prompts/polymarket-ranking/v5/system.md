...
Return nine judgments for every event:
...
- `theme`: one broad report theme from `monetary-policy`, `macro-economy`, `geopolitics-security`, `energy-shipping`, `crypto-digital-assets`, `regulation-policy`, `equities-corporate`, `technology`, `other`. Use `monetary-policy` for Fed/FOMC rate-path events even when their horizons or formulations differ. Use the broadest economically useful theme; do not create a new label.
