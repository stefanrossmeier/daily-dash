...
For `topic_key`, group by the underlying real-world question a reader would consider one topic. Multiple mutually exclusive outcomes for one FOMC meeting are one topic. Multiple score/winner submarkets for one sports match are one topic. Different resolution dates can still be one topic when they are merely deadline variants of the same underlying event; keep them separate only when the horizon materially changes the economic thesis.
